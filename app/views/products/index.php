                    ...
                    <?php foreach($products as $p): ?>
                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                            ...
                            <td style="padding: 15px 12px;">
                                ...
                                <div style="font-size: 0.75rem; color: var(--text-muted);"><?= htmlspecialchars($p['category_name'] ?: 'Sin Categoria') ?></div>
                            </td>
                            <td style="padding: 15px 12px;">
                            ...
                            <td style="padding: 15px 12px;">
                                <span style="background: <?= $p['stock'] > 0 ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)' ?>; color: <?= $p['stock'] > 0 ? '#10b981' : '#ef4444' ?>; padding: 4px 10px; border-radius: 8px; font-size: 0.8rem; font-weight: 700;">
                                    <?= (int)$p['stock'] ?> <span style="font-weight: 400; opacity: 0.7;">unds.</span>
                                </span>
                            </td>
                            <td style="padding: 15px 12px; text-align: right;">
                                ...
                                <form action="products.php?action=delete" method="POST" style="display: inline; margin-left: 10px;" onsubmit="return confirm('¿Eliminar este producto? Esta acción no se puede deshacer.');">
            ...
            <?php foreach($products as $p): ?>
                <div class="mobile-card">
                    ...
                    <div class="mobile-card-meta">
                        <strong>Categoría:</strong> <?= htmlspecialchars($p['category_name'] ?: 'Sin Categoría') ?><br>
                        <?php $currency = $p['currency'] ?? 'CLP'; ?>
                    ...
                    <div class="mobile-card-actions">
                        ...
                        <form action="products.php?action=delete" method="POST" style="flex: 1;" onsubmit="return confirm('¿Eliminar este producto? Esta acción no se puede deshacer.');">
                            <?= \Core\Security::csrfField() ?>
                            <input type="hidden" name="id" value="<?= (int)$p['id'] ?>">
    ...
    <div class="glass-card" style="flex: 1; align-self: flex-start; padding: 25px;">
        ...
        <form action="products.php?action=<?= !empty($editing) ? 'update' : 'store' ?>" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="category_id">
                        ...
                        <?php foreach($categories as $cat): ?>
                            <option value="<?= (int)$cat['id'] ?>" <?= !empty($editing) && (int)($editing['category_id'] ?? 0) === (int)$cat['id'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>Stock</label>
                    <input type="number" name="stock" value="<?= htmlspecialchars($editing['stock'] ?? '0') ?>">
                </div>
                <div class="form-group">
